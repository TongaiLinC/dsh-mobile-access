...
export function apply(ctx) {
  ...
  async function urlsOf() {
    ...
    const lan = net.lan.map(ip => 'http://' + ip + ':' + port)
    const vpn = []
    if (net.tailscaleIp) vpn.push('http://' + net.tailscaleIp + ':' + port)
    if (net.tailscaleHost) vpn.push('https://' + net.tailscaleHost + '/')
    for (const ip of net.zerotier) vpn.push('http://' + ip + ':' + port)
    const proxy = state.policy.proxyPort ? net.lan.map(ip => 'http://' + ip + ':' + state.policy.proxyPort) : []
