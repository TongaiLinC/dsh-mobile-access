// dsh-mobile-access —— DeepSeek Harness 移动端访问插件（静态 cordis 插件形态）
// 供 `dsh plugin --profile web add dsh-mobile-access`（或 package.json link: 依赖）安装；
// 与动态版（仓库根 dsh-mobile-access.js）功能同源，区别：
//   - 静态插件运行在宿主进程：直接使用 node:fs / process / node:os，
//     不再依赖 fs 服务与进程探测；
//   - 仅 Host 半（无 client 半）：全部浏览器端 UI 通过 webServer.tapIndex
//     注入 /dsh-mobile/api/boot.js 实现，门禁页为 /dsh-mobile/gate.html。
//
// 功能：
//   1. PC 端审批门禁：手机设备首次访问必须由 PC 端批准后才能使用；
//   2. 局域网 / 外网自动识别与模式切换：检测访问方式变化并在手机端弹窗询问；
//   3. 外网访问建议/强制使用 Tailscale、ZeroTier 等 VPN；
//   4. 网关代理：监听 0.0.0.0:<proxyPort>，转发到主服务器（改写 Host/Origin
//      通过 /api 信任围栏），未批准设备 302 到门禁页，WebSocket 升级完整转发；
//   5. 窄屏 UI 适配：移动端布局、输入栏分行、自动禁用第三方皮肤。
//
// 状态持久化到 $DSH_HOME/dsh-mobile/state.json（设备、策略、模式选择）。
...
var CSS = [
  ...
  /* 三方插件（dsh-balance-plugin）弹窗层级提到最大、显式视口尺寸、合成层提升
     （配合 boot.js 的定位补偿：产品输入卡片带 backdrop-filter（毛玻璃），会
     劫持 fixed 的 containing block，弹窗被定位到输入卡片处导致「显示不全」；
     iOS Safari 另叠加滚动容器劫持。height 先给 100vh 兜底（旧 iOS 无 dvh）） */
  ...
  /* 皮肤（dsh-deep-whale 等）角色层：窄屏左侧角色隐藏；右侧角色缩小并移到
     左下角（点击穿透），避免遮挡右上角余额/用量等状态区域（v24 起窄屏直接
     禁用第三方皮肤，以下规则仅在其他皮肤/宽屏场景兜底） */
  ...
  /* 产品设置模态：手机上全屏 + 纵向布局——导航变顶部横向滚动条、内容占满
     ...
     不透明），且皮肤会把 --dsw-alias-bg-base 在部分作用域定义为 transparent，
     var() 会解析成透明；v23 起直接用静态双主题色（浅色近白 / 深色深蓝黑），
     保证完全不透出对话背景；presentation 覆盖层 z-index 提到最大 */
...
/* 三方插件弹窗层级与定位修复：dsh-balance-plugin 的 .bmon-overlay 内联渲染在
   输入栏内（无 portal）。产品输入卡片带 backdrop-filter（毛玻璃），
   backdrop-filter / will-change / filter / contain 都会改变 fixed 元素的
   containing block 并创建层叠上下文（Chrome 与 Safari 行为一致），两个后果：
   ① 弹窗被定位到输入卡片处（屏幕中下部），上半屏露出页面；
   ② 弹窗的 z-index 只在输入卡片的层叠上下文内有效，相对根层叠的级别被降为
      输入卡片的级别——GUI 原生侧边栏 / 顶栏（根上下文，z-index 更高）会盖在
      弹窗之上（iOS 实测弹窗被侧边栏顶栏覆盖）。
   v30 处理：弹窗存在期间，把祖先链上的劫持属性（backdrop-filter / filter /
   will-change / contain）临时置为 none（原值存入 data 属性），fixed 随即恢复
   视口定位、z-index 回到根层叠——弹窗天然全屏且在最顶层；弹窗关闭后恢复原值。
   v29 的位置补偿（top/left = -基点位置）保留为兜底：剩余劫持机制（transform
   祖先、iOS Safari 滚动容器）仍有偏差时把弹窗拉回视口原点。 */
function fixBalanceOverlayPosition() {
  ...
  /* ① 解除祖先劫持：backdrop-filter/filter/will-change/contain 置 none；
     ...
     根层叠级别压到其 z 值，低于产品顶栏（z-index 20）——弹窗被顶栏覆盖。
     弹窗打开期间把这些 z-index 临时提升到最大，关闭后恢复。
      transform 不动（产品布局依赖它）；若剩余 transform 劫持，由 ② 补偿位置。 */
...
function forward(req, res, upgradeSocket, upgradeHead) {
  ...
  /* agent:false 让升级请求走独立连接；默认 keep-alive 连接池复用会
     导致 WebSocket 握手错乱（Parse Error / 挂起） */
  var preq = http.request(MAIN + req.url, { method: req.method, headers: headers, agent: false }, function (pres) {
    if (res) {
      /* v1.0.4：HTML 响应强制 no-store。DSH 主服务器对 index.html 没有缓存头，
         iOS Safari 会启发式缓存旧页面，引用的旧 bundle 文件名在新构建后 404，
         导致「空白页 + 徽标正常」而 IP 直连却正常（不同 origin 缓存独立）。 */
      if (pres.headers && String(pres.headers['content-type'] || '').indexOf('text/html') === 0) {
        pres.headers['cache-control'] = 'no-store'
      }
...
// ════════════════════════════════════════════════════════════════════════
// 插件主体（静态形态：export apply；宿主进程内 process / node:fs 可用）
// ════════════════════════════════════════════════════════════════════════
...
export function apply(ctx) {
  ...
  const persistPath = path.join(process.env.DSH_HOME || path.join(os.homedir(), '.dsh'), 'dsh-mobile', 'state.json')
  ...
  function workdir() { return process.cwd() }

  // ────────────────────── 持久化（node:fs 直写） ──────────────────────
  function persist() {
    persistChain = persistChain.then(async () => {
      try {
        await fsp.mkdir(path.dirname(persistPath), { recursive: true })
        await fsp.writeFile(persistPath, JSON.stringify({ v: 1, devices: state.devices, policy: state.policy }), 'utf8')
      } catch (e) {}
    })
  }
  async function loadPersisted() {
    try {
      const text = await fsp.readFile(persistPath, 'utf8')
  ...
  async function urlsOf() {
    ...
    /* LAN/VPN 地址一律走网关代理端口 proxyPort（主服务只绑 127.0.0.1，
       ...
       boot.js 中 showModePopup 会对同源地址跳过「切换」弹窗，这里返回
       proxy 地址作为「切换目标」时能正确区别。 */
  ...
  // ────────────────────── 请求处理 ──────────────────────
  // 注意：宿主沙箱不提供 URL 全局，这里手工解析 pathname。
  ...
  async function statePayload(req) {
    ...
    return {
      ...
      debug: { persistPath, form: 'static-bundle' },
    }
  }
  ...
  ctx.effect(() => webServer.tapIndex(html => {
    let out = html
    // v1.0.4：禁止浏览器/启发式缓存 index.html，避免 iOS Safari 缓存旧页面
    // （旧 bundle 文件名 404 → 空白页）；与网关响应头 no-store 双保险。
    const cacheMeta = '<meta http-equiv="Cache-Control" content="no-store, no-cache, max-age=0">'
    if (out.indexOf('http-equiv="Cache-Control"') < 0) {
      const cmAt = out.toLowerCase().indexOf('<head>')
      if (cmAt >= 0) out = out.slice(0, cmAt + 6) + cacheMeta + out.slice(cmAt + 6)
      else out = cacheMeta + out
    }
